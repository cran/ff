...
\examples{
   ...
   cat("lets return the combined expressions as a new ff object (x unchanged)\n")
   ffrowapply(2*x[i1:i2, ], X=x, RETURN=TRUE, BATCHSIZE=200)
   cat("lets return a single row aggregate\n")
   ffrowapply(t(apply(x[i1:i2,,drop=FALSE], 1, mean)), X=x, RETURN=TRUE, RETCOL=NULL, BATCHSIZE=200)
   cat("lets return a 6 column aggregates\n")
